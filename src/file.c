...
int8_t file_open(char *path, uint8_t mode) {
    ...
    uint8_t fileNameSize = strlen(path) & 0x7f;
            ...
            for (int8_t i = 0; i < FILE_SIZE; i++) {
                if (files[i].name == NULL) {
                    ...
                    if (mode == FILE_OPEN_MODE_WRITE) {
                        ...
                        uint16_t fileEntryAddressData = files[i].entry + 1 + otherFileNameSize;
                        eeprom_write_word(fileEntryAddressData, files[i].address);
                        eeprom_write_word(fileEntryAddressData + 2, files[i].size);
                    }
    ...
    if (mode == FILE_OPEN_MODE_WRITE || mode == FILE_OPEN_MODE_APPEND) {
        ...
        for (uint8_t i = 0; i < fileNameSize; i++) {
            eeprom_write_byte(fileEntryAddress + 1 + i, path[i]);
        }
...
int16_t file_read(int8_t file, uint8_t *buffer, int16_t size) {
    if (file >= 0 && file < FILE_SIZE && files[file].name != NULL) {
        ...
        for (int16_t i = 0; i < size && files[file].position < files[file].size; i++) {
            buffer[i] = eeprom_read_byte(files[file].address + files[file].position++);
            bytes_read++;
        }
        return bytes_read;
    }
...
int16_t file_write(int8_t file, uint8_t *buffer, int16_t size) {
    if (file >= 0 && file < FILE_SIZE && files[file].name != NULL) {
        ...
        for (int16_t i = 0; i < size; i++) {
            eeprom_write_byte(files[file].address + files[file].position++, buffer[i]);
            bytes_writen++;
        }

        uint16_t entry_address_Data = files[file].entry + 1 + eeprom_read_byte(files[file].entry);
        eeprom_write_word(entry_address_Data, files[file].address);
        eeprom_write_word(entry_address_Data + 2, files[file].size);
